...
import type { PhotographyProduct, SelectOption } from '~/api/product/type'

// Props & Emits
const props = defineProps<{
  show: boolean
  photography: PhotographyProduct | null
  isEdit: boolean
}>()
...
const modalTitle = computed(() => {
  if (viewMode.value)
    return '查看婚纱摄影详情'
  return props.photography?.id ? '编辑婚纱摄影' : '新增婚纱摄影'
})

// Form data
const formData = reactive<PhotographyProduct>({
  merchantId: 0,
  productName: '',
  categoryId: 0,
...
// Watches
watch(() => props.photography, (newPhotography) => {
  if (newPhotography) {
    resetForm()
    Object.assign(formData, newPhotography)

    // 处理服务内容
    if (newPhotography.servicesMap) {
      basicServices.value = newPhotography.servicesMap['基础服务'] || []
      valueAddedServices.value = newPhotography.servicesMap['增值服务'] || []
    }

    // 处理主图文件列表
    if (newPhotography.mainImage) {
      mainImageFileList.value = [{
        id: 'mainImage',
        name: getFileNameFromUrl(newPhotography.mainImage),
        status: 'finished',
        url: newPhotography.mainImage,
      }]
    }

    // 处理附加图片文件列表
    if (newPhotography.subImagesList && newPhotography.subImagesList.length > 0) {
      subImagesFileList.value = newPhotography.subImagesList.map((url, index) => ({
        id: `subImage_${index}`,
...
async function fetchMerchantOptions() {
  try {
    const response = await getMerchantList(ProductType.PHOTOGRAPHY)
...
async function handleSubmit() {
  formRef.value?.validate(async (errors) => {
    ...
    try {
      ...
      if (props.photography?.id) {
        // 更新
        const response = await updateProduct(props.photography.id, formData)
        if (response.success) {
      ...
      <NTabs v-model:value="activeTab" type="line" animated>
        ...
        <NTabPane name="basic" tab="基本信息">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <NFormItem label="套餐名称" path="productName" required>
        ...
        <!-- 摄影特有属性 Tab -->
        <NTabPane name="attributes" tab="摄影特性">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
